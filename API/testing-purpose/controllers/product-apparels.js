...
const ApparelProduct = require("../models/product-apparel");
...
exports.products_create_product = (req, res, next) => {
  const product = new ApparelProduct({
...
exports.products_update_product = (req, res, next) => {
  const id = req.params.productId;
  ApparelProduct.findByIdAndUpdate(id, { $set: req.body }, { new: true })
    .exec()
    .then((result) => {
      res.status(200).json({
...
exports.add_product_review = (req, res, next) => {
  ...
  ApparelProduct.findById(productId)
    .then((product) => {
      if (!product) {
...
exports.products_delete_product = (req, res, next) => {
  const id = req.params.productId;
  ApparelProduct.deleteOne({ _id: id })
    .exec()
    .then((result) => {
      res.status(200).json({
